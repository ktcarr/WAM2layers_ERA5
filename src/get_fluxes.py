...
def data_path(yearnumber,a):
    ...
    save_path = join(interdata_folder, str(yearnumber) + '-' + str(a) + 'fluxes_storages.mat')
    
    return sp_data,sp_eoy_data,q_f_data,q_f_eoy_data,tcw_data,tcw_eoy_data,u_f_data,u_f_eoy_data, v_f_data,v_f_eoy_data, ewvf_data, ewvf_eoy_data,nwvf_data, nwvf_eoy_data, eclwf_data, eclwf_eoy_data, nclwf_data, nclwf_eoy_data, ecfwf_data, ecfwf_eoy_data, ncfwf_data, ncfwf_eoy_data,evaporation_data,  precipitation_data,save_path

def load_data(varname, fp, fp_next,  latitude, longitude, begin_time, count_time, is_final_time=False):
    '''Load specified data based, including handling end-of-year stuff'''
    data = xr.open_dataset(fp)[varname]
    data['longitude'] = fix_lon(data['longitude'].values) # make sure longitudes are positive
...
def getPres(latitude, longitude, final_time,a,begin_time,count_time, top_level=100, n_levels=37):
    '''Get pressure levels, after adjusting for surface pressure. Also get pressure difference between levels'''
    # Get surface pressure
    is_final_time = (a==final_time)
    args = ('sp', datapath[0], datapath[1], latitude, longitude, begin_time, count_time, is_final_time)
    ...
    p  = np.stack([sp - n*dp for n in np.arange(n_levels)[::-1]],axis=1) # compute pressure based on dp
...
def getW(q, dp, latitude, longitude, final_time, a, begin_time, count_time, density_water, g, A_gridcell, boundary):
    '''Load W data'''
    is_final_time = (a==final_time)
    args    = ('tcw', datapath[4], datapath[5], latitude, longitude, 
               begin_time, count_time, is_final_time)
    tcw_ERA = load_data(*args).values  
    
    # Compute column water vapor based on defined pressure levels
    q_  = (q[:,1:] + q[:,:-1])/2 # estimate average specific humidity between pressure levels
    cwv = q_ * dp[:,None,...] / g
    tcwv = np.sum(cwv,axis=1) #total column water vapor, cwv is summed over the vertical [kg/m2]
    ...
    vapor      = vapor_top + vapor_down + eps # add small number to avoid divide-by-zero below
...
#%% Code
def getFa(latitude,longitude,boundary,cw,U,V,count_time,begin_time,yearnumber,a,final_time):
    ...
    U_ = (U[:,1:] + U[:,:-1])/2 # interpolate velocities to match column water estimates
    V_ = (V[:,1:] + V[:,:-1])/2
    Fa_E_p = U_ * cw
    Fa_N_p = V_ * cw
    ...
    for i in range(len(longitude)):
        for j in range(len(latitude)):
            for k in range(count_time+1):
                corr_E[k,j,i] = min(2,max(0,ewf[k,j,i]/(Fa_E_down_uncorr[k,j,i] + Fa_E_top_uncorr[k,j,i] + eps)))
                corr_N[k,j,i] = min(2,max(0,nwf[k,j,i]/(Fa_N_down_uncorr[k,j,i] + Fa_N_top_uncorr[k,j,i] + eps))) 
...
def getEP(latitude,longitude,yearnumber,begin_time,count_time,A_gridcell):
    ...
    load = lambda varname, fp : load_data(varname, fp, None, *args).isel(time=slice(None,count_time*3))
    ...
    evaporation = np.reshape(np.abs(np.minimum(np.reshape(evaporation, (np.size(evaporation))),0.0)),(int(count_time*3),len(latitude),len(longitude)))   
...
def getrefined(Fa_E_top,Fa_N_top,Fa_E_down,Fa_N_down,W_top,W_down,E,P,divt,count_time,latitude,longitude):
    ...
    E_small = np.nan*np.zeros((int(count_time*3*divt2),len(latitude),len(longitude)))
    for t in range(1,int(count_time*3*divt2)+1):
        E_small[t-1] = (1./divt2) * E[int(t/divt2+oddvector2[0,t-1]-1)]
    E = E_small            

    P_small = np.nan*np.zeros((int(count_time*3*divt2),len(latitude),len(longitude)))
    for t in range(1,int(count_time*3*divt2)+1):
        P_small[t-1] = (1./divt2) * P[int(t/divt2+oddvector2[0,t-1]-1)]
    P = P_small
    ...
    W_top_small = np.nan*np.zeros((int(count_time*divt+1),len(latitude),len(longitude)))
    for t in range(1,int(count_time*divt)+1):
        W_top_small[t-1] = W_top[int(t/divt+oddvector[0,t-1]-1)] + partvector[0,t-1] * (W_top[int(t/divt+oddvector[0,t-1])] - W_top[int(t/divt+oddvector[0,t-1]-1)])
        W_top_small[-1] = W_top[-1]
    W_top = W_top_small

    W_down_small = np.nan*np.zeros((int(count_time*divt+1),len(latitude),len(longitude)))
    for t in range(1,int(count_time*divt)+1):
        W_down_small[t-1] = W_down[int(t/divt+oddvector[0,t-1]-1)] + partvector[0,t-1] * (W_down[int(t/divt+oddvector[0,t-1])] - W_down[int(t/divt+oddvector[0,t-1]-1)])
        W_down_small[-1] = W_down[-1]
    W_down = W_down_small

    Fa_E_down_small = np.nan*np.zeros((int(count_time*divt),len(latitude),len(longitude)))
    Fa_N_down_small = np.nan*np.zeros((int(count_time*divt),len(latitude),len(longitude)))
    Fa_E_top_small = np.nan*np.zeros((int(count_time*divt),len(latitude),len(longitude)))
    Fa_N_top_small = np.nan*np.zeros((int(count_time*divt),len(latitude),len(longitude)))
    for t in range(1,int(count_time*divt)+1):
...
def get_stablefluxes(W_top,W_down,Fa_E_top_1,Fa_E_down_1,Fa_N_top_1,Fa_N_down_1,
    ...
    #convert to m3
    Fa_E_top = Fa_E_top_kgpmps * timestep/float(divt) * L_EW_gridcell / density_water # [kg*m^-1*s^-1*s*m*kg^-1*m^3]=[m3]
    Fa_E_down = Fa_E_down_kgpmps * timestep/float(divt) * L_EW_gridcell / density_water # [s*m*kg*m^-1*s^-1*kg^-1*m^3]=[m3]

    Fa_N_top_swap = np.zeros((len(latitude),int(count_time*float(divt)),len(longitude)))
    Fa_N_down_swap = np.zeros((len(latitude),int(count_time*float(divt)),len(longitude)))
    ...
    for c in range(len(latitude)):
        Fa_N_top_swap[c] = Fa_N_top_kgpmps_swap[c] * timestep/float(divt) * 0.5 *(L_N_gridcell[c]+L_S_gridcell[c]) / density_water # [s*m*kg*m^-1*s^-1*kg^-1*m^3]=[m3]
        Fa_N_down_swap[c] = Fa_N_down_kgpmps_swap[c] * timestep/float(divt) * 0.5*(L_N_gridcell[c]+L_S_gridcell[c]) / density_water # [s*m*kg*m^-1*s^-1*kg^-1*m^3]=[m3]
    ...
    Fa_E_top_stable = np.reshape(np.minimum(np.reshape(Fa_E_top_abs, (np.size(Fa_E_top_abs))), (np.reshape(Fa_E_top_abs, (np.size(Fa_E_top_abs)))  / 
                                    (eps + np.reshape(Fa_E_top_abs, (np.size(Fa_E_top_abs)))  + np.reshape(Fa_N_top_abs, (np.size(Fa_N_top_abs))))) * stab 
                                            * np.reshape(W_top[:-1,:,:], (np.size(W_top[:-1,:,:])))),(int(count_time*float(divt)),len(latitude),len(longitude)))
    ...
    Fa_E_down_stable = np.reshape(np.minimum(np.reshape(Fa_E_down_abs, (np.size(Fa_E_down_abs))), (np.reshape(Fa_E_down_abs, (np.size(Fa_E_down_abs)))  / 
                                    (eps + np.reshape(Fa_E_down_abs, (np.size(Fa_E_down_abs)))  + np.reshape(Fa_N_down_abs, (np.size(Fa_N_down_abs))))) * stab 
                                             * np.reshape(W_down[:-1,:,:], (np.size(W_down[:-1,:,:])))),(int(count_time*float(divt)),len(latitude),len(longitude)))
...
def getFa_Vert(Fa_E_top,Fa_E_down,Fa_N_top,Fa_N_down,E,P,W_top,W_down,divt,count_time,latitude,longitude):
    ...
    Fa_Vert_stable = np.reshape(np.minimum(np.reshape(Fa_Vert_abs, (np.size(Fa_Vert_abs))), np.minimum(stab*np.reshape(W_top[1:,:,:], (np.size(W_top[1:,:,:]))), 
                                        stab*np.reshape(W_down[1:,:,:], (np.size(W_down[1:,:,:]))))),(int(count_time*float(divt)),len(latitude),len(longitude)))
