...
#     Function returns a mask with with 1s representing the area inside of path
    ...
    t = zip(lon_lat_grid[0].flatten(), lon_lat_grid[1].flatten()) # get pairs of lat/lon
...
def get_Sa_track_backward(latitude,longitude,count_time,divt,Kvf,Region,Fa_E_top,Fa_N_top,Fa_E_down,Fa_N_down,
    ...
    for t in np.arange(int(count_time*divt),0,-1):
        ...
        + Fa_E_down_WE[t-1,1:-1,:] * (Sa_track_E_down[0,1:-1,:] / Sa_E_down[0,1:-1,:]) #         (above) Look at cell to the East: compute ratio of tracked to total moisture, and multiply by the flux going west-to-east across the eastern boundary (we are going backwards, so adding this quantity gives us the tracked moisture in the center cell from the previous time step. Repeat for the other boundaries 
        - Fa_E_down_EW[t-1,1:-1,:] * (Sa_track_down[t,1:-1,:] / W_down[t,1:-1,:]) #         (above) Similary, look at center cell: compute ratio of tracked moisture to total moisture, and look at flux going east to west across the eastern boundary. In reverse, this flux goes west to east, so multiply this ratio by the ratio of moisture in the center cell to get the moisture in the current cell for the the previous time step.
...
def create_empty_array(count_time,divt,latitude,longitude,year,doy):
    ...
    Sa_track_down = np.zeros((int(count_time*divt)+1,len(latitude),len(longitude)))       
    sio.savemat(os.path.join(sub_interdata_folder, f'{year}-{doy}Sa_track.mat'),\
                {'Sa_track_top':Sa_track_top, 'Sa_track_down': Sa_track_down,} ,do_compression=True) 
    return
...
if __name__=='__main__':
    ...
    parser.add_argument('--Kvf',             dest='Kvf', default=3., type=float)
    ...
    create_empty_array(count_time-1,divt,latitude,longitude,year_start,doy_start) #creates empty arrays for first day run
    ...
    for a in thisyearpart:  
        ...
        if timetracking == 0:
            Sa_track_top,Sa_track_down,north_loss,south_loss,down_to_top,top_to_down,water_lost = get_Sa_track_backward(latitude,longitude,count_time_,divt,Kvf,Region,Fa_E_top,Fa_N_top,Fa_E_down,Fa_N_down,
                                       Fa_Vert,E,P,W_top,W_down,Sa_track_top_last,Sa_track_down_last)
        elif timetracking == 1:
            ...
            Sa_time_top,Sa_time_down,Sa_track_top,Sa_track_down,north_loss,south_loss,down_to_top,top_to_down,water_lost = get_Sa_track_backward_TIME(latitude,longitude,count_time_,divt,timestep,Kvf,Region,Fa_E_top,Fa_N_top,Fa_E_down,Fa_N_down, Fa_Vert,E,P,W_top,W_down,Sa_track_top_last,Sa_track_down_last,Sa_time_top_last,Sa_time_down_last)
